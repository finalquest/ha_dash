import { useMutation, useQueryClient } from '@tanstack/react-query';
import type { HaEntity } from '../api/types';
import { toggleLight, setLightBrightness } from '../api/client';

export const useLightControl = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (entityId: string) => toggleLight(entityId),
    onMutate: async (entityId: string) => {
      await queryClient.cancelQueries({ queryKey: ['entities'] });
      const previous = queryClient.getQueryData<HaEntity[]>(['entities']);
      queryClient.setQueryData<HaEntity[]>(['entities'], (current) => {
        if (!current) return current;
        return current.map((entity) => {
          if (entity.entity_id !== entityId) {
            return entity;
          }
          const nextState = entity.state === 'on' ? 'off' : 'on';
          const timestamp = new Date().toISOString();
          return {
            ...entity,
            state: nextState,
            last_changed: timestamp,
            last_updated: timestamp,
          };
        });
      });
      return { previous };
    },
    onError: (_error, _entityId, context) => {
      if (context?.previous) {
        queryClient.setQueryData(['entities'], context.previous);
      } else {
        queryClient.invalidateQueries({ queryKey: ['entities'] });
      }
    },
  });
};

export const useLightBrightness = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ entityId, percentage }: { entityId: string; percentage: number }) =>
      setLightBrightness(entityId, percentage),
    onMutate: async ({ entityId, percentage }) => {
      await queryClient.cancelQueries({ queryKey: ['entities'] });
      const previous = queryClient.getQueryData<HaEntity[]>(['entities']);
      const normalizedPercentage = Math.max(0, Math.min(100, Math.round(percentage)));
      const brightnessValue = Math.round((normalizedPercentage / 100) * 255);
      queryClient.setQueryData<HaEntity[]>(['entities'], (current) => {
        if (!current) return current;
        return current.map((entity) => {
          if (entity.entity_id !== entityId) {
            return entity;
          }
          const timestamp = new Date().toISOString();
          return {
            ...entity,
            state: normalizedPercentage <= 0 ? 'off' : 'on',
            last_changed: normalizedPercentage <= 0 ? timestamp : entity.last_changed,
            last_updated: timestamp,
            attributes: {
              ...entity.attributes,
              brightness: brightnessValue,
              brightness_pct: normalizedPercentage,
            },
          };
        });
      });
      return { previous };
    },
    onError: (_error, _variables, context) => {
      if (context?.previous) {
        queryClient.setQueryData(['entities'], context.previous);
      } else {
        queryClient.invalidateQueries({ queryKey: ['entities'] });
      }
    },
  });
};
